...
object ShockWaveLegend: ItemSkill("shock_wave", 75, IItemRarity.LEGEND, 3){
	...
	override fun serverFunction(world: World, player: EntityPlayer, handIn: EnumHand) {
		...
		GlobalScope.launch {
			...
			(world as WorldServer).addScheduledTask(){
				for (i in entityList) {
					i.attackEntityFrom(EarthenDamage(player), (player.getATK().toFloat() + player.fallDistance) * 3F)
				}
			}
		}
	}
